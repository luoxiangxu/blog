...
public class PostServiceImpl implements PostService{
    ...
    public Post updatePost(UUID id, UpdatePostRequest updatePostRequest) {
        ...
        existPost.setReadingTime(calculateReadingTime(updatePostRequest.getContent()));

        UUID updatePostRequestCategoryId = updatePostRequest.getCategoryId();
        if(!existPost.getCategory().getId().equals(updatePostRequestCategoryId)){
            Category newCategory = categoryService.getCategoryById(updatePostRequestCategoryId);
            existPost.setCategory(newCategory);
        }

        Set<UUID> existingTagIds = existPost.getTags().stream().map(Tag::getId).collect(Collectors.toSet());
        Set<UUID> updatePostRequestTagIds = updatePostRequest.getTagIds();
        if(!existingTagIds.equals(updatePostRequestTagIds)){
            List<Tag> newTags = tagService.getTagByIds(updatePostRequestTagIds);
            existPost.setTags(new HashSet<>(newTags));
        }
    ...
    public Post getPost(UUID id) {
        return postRepository.findById(id)
            .orElseThrow(() -> new EntityNotFoundException("Post does not exit with id " +id));
    }

    @Override
    public void deletPost(UUID id) {
